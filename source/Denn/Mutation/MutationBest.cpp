...
namespace Denn
{
	...
	public:
		...
		) override
		{
			...
			auto& x_d    = parent(rand_deck.get_random_id(id_best)).array();
			output.array() = (x_best + ((x_a - x_b) + (x_c - x_d)) * output.f()).unaryExpr(solver().clamp_function());
		}
	};
