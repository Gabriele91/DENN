	...
	public:
		...
		) override 
		{
			...
			for(long k=-nn; k!=(nn+1); ++k)
			{
				...
				if(loss_function_compare(population.eval(i), population.eval(id_l_best))) 
					id_l_best = i;
			}
			//get generator
			...
			auto& w_g_best = population[population.best_id()];
			auto& w_l_best = population[id_l_best];
			auto& x_g_a = population[rand_deck.get_random_id(id_target)];
			auto& x_g_b = population[rand_deck.get_random_id(id_target)];
			auto& x_l_a = population[rand_deck_ring_segment.get_random_id()];
			auto& x_l_b = population[rand_deck_ring_segment.get_random_id()];
			//global
			...
			//lambda = 0 -> l_m
			Scalar* w_final_array = (*output).data();
			Scalar* w_g_m_array   = g_m.data();
			Scalar* w_l_m_array   = l_m.data();
			for(Matrix::Index i=0; i != (*output).size(); ++i)
